...
#                        __________XXZ-model HVA adiabatic evolution___________
# 

# In this script we implement the adiabatically-assited VQE for the XXZ model using an HVA ansatz.
...
def optimization(simulation_parameters,max_iterations,conv_tol,step_size, Delta_list):
    ...
    gamma_val = 0.005*jnp.ones(shape=(4*layers,))
    initial_params = [gamma_val]
    ...
    for Delta in Delta_list:
        ...
        def cost(params):
            gamma_ZZ_odd = params[:layers]
            gamma_XY_odd = params[layers:2*layers]
            gamma_ZZ_even = params[2*layers:3*layers]
            gamma_XY_even = params[3*layers:]
            for i in range(N):
                qml.PauliX(wires=[i])
                if(i%2)==0:
            ...
            for layer in range(layers):
                for i in np.arange(1, N-1, 2):
                    qml.IsingZZ(gamma_ZZ_odd[layer], wires=[i, i+1])
                    qml.IsingYY(gamma_XY_odd[layer], wires=[i, i+1])
                    qml.IsingXX(gamma_XY_odd[layer], wires=[i, i+1])
                for i in np.arange(0, N, 2):
                    qml.IsingZZ(gamma_ZZ_even[layer], wires=[i, i+1])
                    qml.IsingYY(gamma_XY_even[layer], wires=[i, i+1])
                    qml.IsingXX(gamma_XY_even[layer], wires=[i, i+1])
