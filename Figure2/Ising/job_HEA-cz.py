...
#                        __________Ising-model CZ adiabatic evolution___________
# 
# In this script we implement an adiabatically assisted VQE with a HEA with entangling CZ gates
#  to an Ising model. In this version of the script, the cost function is adiabatically modified from a
# weakly interacting short-range Ising model to the critical point.
# In this case, the optimization is done with jax to obtain a superior performance.
# 
# INPUTS:
...
def save(output,simulation_parameters):
    ...
    store_dir = '/mnt/netapp1/Store_CSIC//home/csic/qia/ctl/Variational_Waveguide/paper/figure1/'
    import pickle 
    name = 'hea-cz_adiab'
...
def optimization(simulation_parameters,max_iterations,conv_tol,step_size,xi_list,gamma_list):
    ...
    key1 = random.PRNGKey(632021)  # Random seed is explicit in JAX
    thetasz1 = 0.01*2*pi*random.uniform(key1,shape=(int(N*(layers)),))
    key2 = random.PRNGKey(6032021)  # Random seed is explicit in JAX
    thetasx = 0.01*2*pi*random.uniform(key2, shape=(N*layers,))
    key3 = random.PRNGKey(362021)  # Random seed is explicit in JAX
    thetasz2 = 0.01*2*pi*random.uniform(key3,shape=(N*layers,))
    initial_params = [thetasz1,thetasx,thetasz2]
